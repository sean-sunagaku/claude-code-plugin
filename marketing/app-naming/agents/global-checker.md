...
4. 各候補について以下をチェック（WebSearch で調査）:
   - **日本語カタカナでの読みやすさ**（最重要。カタカナ表記を必ず併記）
   ...
   **過去の脱落パターン（必ずチェック）:**
   - カタカナに「ヴ」が入る名前は日本語話者に読みにくい → 即警告
   - 異なる綴りでカタカナが同一になる候補ペアを検出する（例: Velora/Verola → 両方「ヴェロラ」）
   - スペイン語の "no va" 分解（例: Knova）、フランス語普通名詞との一致（例: Volant）を特に注意
   - 既存の有名言語の普通名詞と完全一致する名前は除外推奨
