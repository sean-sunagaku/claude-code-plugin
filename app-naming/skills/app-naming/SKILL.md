...
name: app-naming
description: >
  アプリ名・サービス名の命名を、6つの専門エージェントチームで多角的に評価・決定するスキル。
  ブランディング、商標/法的リスク、デジタルプレゼンス(SEO/ASO/SNS)、国際展開(多言語/発音)の
  4観点から候補を提案・調査・議論し、要所でユーザーの好み・判断を確認しながら
  コンテキストファイルと議事録で次回セッションへ引き継ぐ。
...
6つの専門エージェントがチームで相互フィードバックしながら議論し、最適なアプリ名を決定する。
要所でユーザーに質問を投げて好み・判断を確認し、チームの議論に反映する。
...
3. コンテキスト作成 → 候補提案 → **ユーザー確認** → 相互フィードバック → 絞り込み → **ユーザー最終決定**
...
TeamCreate で `app-naming` チームを作成。以下6エージェントを **1つのメッセージで並列に** Task ツールで起動する。
...
| `user-liaison` | ユーザーへの質問を一元管理、回答をチームに共有 | AskUserQuestion |
...
6. user-liaison: ユーザーへの質問（候補提案後・絞り込み後・最終決定前の3チェックポイント）
7. 統合レポート + 最終候補確定: タスク1〜6完了後（`addBlockedBy: ["1","2","3","4","5","6"]`）
...
1. **自分の調査が終わったら即座に全員へ共有**: 全6エージェントに SendMessage で送る
...
5. **ユーザーに確認したいことがあれば user-liaison に依頼する**: user-liaison が質問を整理してユーザーに投げる
...
[user-liaison（Checkpoint 1: 候補提案直後）]
  → ユーザーに AskUserQuestion:
    - 「15候補の中で直感的に好きなものは？」（上位候補から選択肢）
    - 「逆に絶対NGなものは？」
  → 全員に: ユーザーの回答を共有
```
...
[user-liaison（Checkpoint 2: 絞り込み後）]
  → ユーザーに AskUserQuestion:
    - 「上位5候補、この方向性で合っていますか？」
    - 「もっとXX寄りの候補が欲しい等あれば教えてください」
  → 全員に: ユーザーの回答を共有
```
...
[user-liaison（Checkpoint 3: 最終決定前）]
  → ユーザーに AskUserQuestion:
    - 「最終候補からどれにしますか？」（トップ3-4を選択肢に）
  → 全員に: ユーザーの最終決定を共有
```
