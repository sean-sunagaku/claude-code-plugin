...
3. **UI 機能網羅チェック**を実施（下記参照）
4. 不足があればユーザーに質問する
5. ファイルに出力
6. ユーザーに確認を求めて停止

#### UI 機能網羅チェック（ステップ2 で必ず実施）

触ってみないとわからない機能漏れを、設計段階で防ぐためのチェック。

**A) CRUD + 並び替えチェック**

データを持つ UI 要素ごとに、5 つの操作が全てカバーされているか確認する：

| 操作 | 確認項目 |
|------|---------|
| **Create** | 追加の UI がある。空状態でも追加できる |
| **Read** | 一覧表示がある。空状態の表示がある |
| **Update** | インライン編集 or 編集画面がある。タップで編集モードに入れる |
| **Delete** | 削除ボタンが見える位置にある（context menu だけでは不十分） |
| **Reorder** | ドラッグ&ドロップ or 並び替え手段がある |

漏れがあれば `02_screen-info.md` に操作を追加し、ステップ3以降に反映する。

**B) 操作動詞チェック**

UI 要素ごとに「ユーザーがしたいこと」を動詞で列挙し、全て実装対象になっているか確認する：

```
例: タスクリスト
  → 追加、表示、編集、削除、並び替え、開始、完了

例: タイマー
  → 開始、一時停止、再開、中断、延長
```

漏れている動詞 = 漏れている機能。`02_screen-info.md` にテーブルとして出力する。

**C) アフォーダンスチェック**

各操作について「初見ユーザーが操作方法を発見できるか」を確認する：
- 削除: ボタンが常に見えるか？（右クリック/スワイプだけでは発見できない）
- 編集: クリック/タップで編集できることが視覚的にわかるか？
- 並び替え: ドラッグハンドル or 明示的 UI があるか？

アフォーダンスが不足している操作は `02_screen-info.md` に警告として記載し、
ステップ4（ワイヤーフレーム）で UI を補完する。
...
- **UI 機能網羅チェック**（ステップ2 完了時）：
  - データを持つ UI 要素に CRUD + 並び替えの5操作が定義されているか
  - 操作動詞テーブルに漏れがないか（動詞が少ない要素を警告）
  - アフォーダンス不足の操作がないか（context menu のみ / スワイプのみの操作を警告）
