...
description: >
  ...
  ペルソナ駆動モードでは、各ペルソナの視点からウォークスルーを実施する。
...
6. **【Mode B】各ペルソナ視点でのウォークスルー**:
   各バリエーションについて、対象ペルソナの立場でシナリオベースのウォークスルーを実施:
   ```
...
### Phase 2: フィードバック
7. **ui-designer と copy-writer に SendMessage でフィードバックを共有**:
   ```
   [チームメンバーへ] アフォーダンス検証結果を共有します。

   初見理解度スコア: X/10
   ペルソナ適合度スコア: X/10（Mode B の場合）
...
- **ペルソナ視点では「この人の立場なら」で判断する。自分の感覚ではなく、ペルソナの状況を想像する**
